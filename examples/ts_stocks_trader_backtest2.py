    ...
    for oneStock in stocks:
        ...
        if not hasCache:
            ...
            for oneEndDate in [date for date in trade_dates if int(date) > fromDate]:
                ...
                barMin = dataCache.pro_bar(ts_code=oneStock,
                                           start_date=trade_dates[trade_dates.index(oneEndDate) - 500],
                                           end_date=oneEndDate)
                # 没有数据的情况
                if len(barMin) == 0:
                    continue
                ...
                if ('日线_60分钟_vg三买' in trader.s.keys()) == False:
                    continue
        ...
        if isBackTestResult:
            ...
            for oneDay in resultDataFrame['日期'].values.tolist():
                ...
                # 前后看100天
                thisDF = resultDataFrame.loc[resultDataFrame['日期'] == oneDay, ['3K形态', '回调幅度', '上攻涨幅', '震荡时间', '最后一笔天数']]
                ...
                currentDay = currentDay.drop(
                    columns=['b1b', 'b2b', 'b3b', 'b5b', 'b8b', 'b13b', 'b21b', 'b34b', 'b55b', 'b89b', 'b144b',
                             'b233b', 'b377b'])

                currentDay['3K形态'] = thisDF['3K形态'].values.tolist()[0].split("-")[0]
                currentDay['回调幅度'] = thisDF['回调幅度'].values.tolist()[0]
                currentDay['上攻涨幅'] = thisDF['上攻涨幅'].values.tolist()[0]
                currentDay['震荡时间'] = thisDF['震荡时间'].values.tolist()[0]
                currentDay['最后一笔天数'] = thisDF['最后一笔天数'].values.tolist()[0]
