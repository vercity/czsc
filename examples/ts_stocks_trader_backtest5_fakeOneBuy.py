...
from czsc.signals.cxt import cxt_vg_fakeOneBuy
...
fromDate = 20150101
strategyName = "vg潜在一买，day" + str(fromDate)
    ...
    def get_signals(cat: CzscAdvancedTrader) -> OrderedDict:
        ...
        for oneFreq in cat.kas.keys():
            ...
            if oneFreq == '日线':
                ...
                if s['日线_倒0笔_方向'].split("_")[0] == '向下':
                    s.update(cxt_vg_fakeOneBuy(cat, "日线"))
                if s['日线_倒0笔_长度'].split("_")[0] == '3根K线':
...
def backtest(stocks):
    ...
    for oneStock in stocks:
        ...
        if oneStock == '600545.SH':
            continue
        oneStock = '300442.SZ'
        resultDataFrame = pd.DataFrame(columns=["日期", "标的", "3K形态", "倒1K状态", "中枢力度对比", "过渡力度对比"])
        print(oneStock)
        ...
        if not hasCache:
            ...
            for oneEndDate in [date for date in trade_dates if int(date) > fromDate]:
                ...
                if ('日线_vg潜在一买' in trader.s.keys()) == False:
                    continue
                sanmaihuicaiSignal = trader.s['日线_vg潜在一买']
                if kCountThree in kCount:
                    if sanmaiqueren in sanmaihuicaiSignal:
                        ...
                        resultDataFrame = resultDataFrame.append(
                            {"日期": otherSignals["日期"], "标的": oneStock, "3K形态":otherSignals["3K形态"], "倒1K状态":otherSignals["倒1K状态"],"中枢力度对比":detailSignal[1], "过渡力度对比":detailSignal[2] }, ignore_index=True)
        ...
        if isBackTestResult:
            ...
            for oneDay in resultDataFrame['日期'].values.tolist():
                ...
                currentDay = probar[probar["trade_date"] == trade_dates[trade_dates.index(oneDay) +1]]
