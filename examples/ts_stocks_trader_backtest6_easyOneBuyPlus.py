...
from czsc.signals.cxt import cxt_vg_easyOneBuy
...
fromDate = 20150101
strategyName = "vg简单一买Plus，day" + str(fromDate)
        ...
        for oneFreq in cat.kas.keys():
            ...
            if oneFreq == '日线':
                ...
                if s['日线_倒0笔_长度'].split("_")[0] == '3根K线' and s['日线_倒0笔_方向'].split("_")[0] == '向上':
                    s.update(cxt_vg_easyOneBuy(cat, "日线"))
...
def backtest(stocks):
    ...
    for oneStock in stocks:
        ...
        if oneStock == '600545.SH' or oneStock == '002664.SZ' or oneStock == '000415.SZ' or oneStock == '002608.SZ' or oneStock == '002694.SZ':
            continue
        resultDataFrame = pd.DataFrame(columns=["日期", "标的", "3K形态", "倒1K状态", "BS1辅助","反转迹象","TAS一买"])
        print(oneStock)
        ...
        if not hasCache:
            ...
            for oneEndDate in [date for date in trade_dates if int(date) > fromDate]:
                ...
                if ('日线_vg简单一买' in trader.s.keys()) == False:
                    continue
                sanmaihuicaiSignal = trader.s['日线_vg简单一买']
