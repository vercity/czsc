...
def bar_vol_bs1_V230224(c: CZSC, di: int = 1, n: int = 20, **kwargs):
    ...
    **信号列表：**
    ...
    short_c1 = _bars[-1].high == max([x.high for x in _bars]) and _bars[-1].upper > 2 * max(_bars[-1].solid, _bars[-1].lower)
    short_c2 = _bars[-1].amount > mean_vol * 3

    long_c1 = _bars[-1].low == min([x.low for x in _bars]) and _bars[-1].lower > 2 * max(_bars[-1].solid, _bars[-1].upper)
    if long_c1:
        print("")
