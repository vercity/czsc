...
from czsc.signals.cxt import cxt_vg_oneBuy
...
fromDate = 20150101
strategyName = "vg一买Plus，60min" + str(fromDate)
    ...
    def get_signals(cls, cat) -> OrderedDict:
        ...
        for oneFreq in cat.kas.keys():
            ...
            if oneFreq == '60分钟':
                ...
                if s['60分钟_倒0笔_长度'].split("_")[0] == '3根K线' and s['60分钟_倒0笔_方向'].split("_")[0] == '向上':
                    s.update(cxt_vg_oneBuy(cat, "60分钟"))
...
def calculateOtherSignals(c):
    ...
    return {"3K形态" : signal,
            "BS1辅助" : bar_vol_bs1_V230224(c.kas['60分钟'])["60分钟_D1N20量价_BS1辅助"].split("_")[0],
            "反转迹象" : bar_reversal_V230227(c.kas['60分钟'])["60分钟_D1A300_反转V230227"].split("_")[0],
            "TAS一买" : tas_first_bs_V230217(c.kas['60分钟'])["60分钟_D1N10SMA5_BS1辅助"].split("_")[0]}
...
def backtest(stocks):
    ...
    for oneStock in stocks:
        if oneStock.endswith('BJ'):
            continue
        if oneStock == '600545.SH':
            continue
        resultDataFrame = pd.DataFrame(columns=["日期", "标的", "3K形态", "倒1K状态", "中枢力度对比", "过渡力度对比", "BS1辅助","反转迹象","TAS一买"])
        print(oneStock)
        ...
        if not hasCache:
            ...
            for oneEndDate in [date for date in trade_dates if int(date) > fromDate]:
                ...
                if len(barMin) == 0:
                    continue
                bg = BarGenerator(base_freq='30分钟', freqs=['60分钟'], max_count=10000)
                for bar in barMin:
                    bg.update(bar)
                trader = CzscTrader(bg=bg, get_signals=CzscStocksCustomBacktest.get_signals)
                print(trader.s['dt'])
                kCount = trader.s['60分钟_倒0笔_长度']
                # zhongshugongzhenSignal = trader.s['日线_60分钟_中枢共振']
                if ('60分钟_vg一买' in trader.s.keys()) == False:
                    continue
                sanmaihuicaiSignal = trader.s['60分钟_vg一买']
                if kCountThree in kCount:
                    if sanmaiqueren in sanmaihuicaiSignal:
                        ...
                        # otherSignals["倒1K状态"] = trader.s["60分钟_倒1K_状态"].split("_")[0]
                        otherSignals["倒1K状态"] = "未知"
                        ...
                        resultDataFrame = resultDataFrame.append(
                            {"日期": otherSignals["日期"], "标的": oneStock, "3K形态":otherSignals["3K形态"],
                             "倒1K状态":otherSignals["倒1K状态"],"中枢力度对比":detailSignal[1], "过渡力度对比":detailSignal[2],
        ...
        if isBackTestResult:
            ...
            for oneDay in resultDataFrame['日期'].values.tolist():
                ...
                # 前后看100天
                thisDF = resultDataFrame.loc[resultDataFrame['日期'] == oneDay, ['3K形态', '中枢力度对比', '过渡力度对比']]
