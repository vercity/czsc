...
def jcc_san_szx_V221122(c: CZSC, di: int = 1, th: int = 10, lastN = 5, appearTimes = 3, **kwargs) -> OrderedDict:
    """三星形态
    ...
    **信号列表：**
    ...
    v1 = "其他"
    if len(c.bars_raw) > lastN + 1 + di:
        bars = get_sub_elements(c.bars_raw, di, n=lastN)
        if sum([check_szx(bar, th) for bar in bars]) >= appearTimes:
            v1 = "满足"
